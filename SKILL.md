...
description: |
  ...
  **当以下情况时使用此 Skill**：
  ...
  (2) 用户要搜索或查看笔记：「搜一下」「找找笔记」「打开某条笔记」「笔记详情」
  (3) 用户要配置话袋笔记：「配置话袋」「连接话袋笔记」「怎么填 Key」
metadata:
  openclaw:
    requires: {}
    optionalEnv:
...
- **开放 API 文档**：`https://ihuadai.cn/openapi`
- **鉴权与必需请求头**：`HUADAI_API_KEY`**（`Authorization`）及 **`HUADAI_USER_UUID`**（`USER_UUID`，与话袋用户 `unique_id` 一致）
- **OAuth（设备码）**：按 `references/oauth.md` 执行；默认使用服务端预注册的固定 `client_id`，一般**只需要**配置 `HUADAI_BASE_URL` 即可走授权。仅在需要覆盖时才配置 `HUADAI_CLIENT_ID`。
...
- `/huadai config` → `references/config.md`
- `/huadai oauth` → `references/oauth.md`
- `/huadai save` → `references/save.md`
- `/huadai search` → `references/search.md`

## 自然语言路由（触发规则）

- 「记/存/收藏/保存到笔记/整理成笔记」→ Save
- 「搜/找/检索/有哪些相关笔记」→ Search
- 「配置/连接/怎么填 key/报错未配置」→ Config

## API 路由表

> 说明：以下为 MVP 参考路径。

- `GET /open/api/v1/search`（搜索笔记）
- `POST /open/api/v1/block/upload-block`（新建笔记 Block）
- `POST /open/api/v1/block/update-content`（更新笔记内容/属性）
